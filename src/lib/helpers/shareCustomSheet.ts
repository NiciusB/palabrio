// Copied and adapted from https://raw.githubusercontent.com/on2-dev/share-api-polyfill/main/src/share.js
...
export default (function () {
	...
	function appendCSS(content: string) {
		var css = content,
			...
			function closeShareUI() {
				...
				const removeContainer = () => {
					...
					document.head.removeChild(
						document.querySelector('#shareAPIPolyfill-style') as Node
					)
...
`
			...
			function keyCloseEvent(event: KeyboardEvent) {
				if (event.keyCode === 27) {
					// ESC
					closeShareUI()
				}
			...
			function bindEvents() {
				Array.from(
					container.querySelectorAll<HTMLElement>('.tool-icon')
				).forEach((tool) => {
						...
						switch (tool.dataset.tool) {
							...
							case 'twitter': {
								const hashTagsStr = hashtags
									...
									: ''
								const urlStr = url ? `url=${encodeURIComponent(url)}` : ''
								const textStr = text ? `text=${encodeURIComponent(text)}` : ''
								window.open(
									`https://twitter.com/intent/tweet?` +
										[textStr, urlStr, hashTagsStr].filter(Boolean).join('&')
								)
								break
							}
							case 'linkedin': {
								window.open(
									`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(
										...
										title ?? ''
									)}&summary=${encodeURIComponent(text ?? '')}&source=LinkedIn`
								)
								break
							}
							case 'telegram': {
